...
\ \ \ \ \ \ \ In February of 2005, consumer data broker ChoicePoint (the Company) ended up paying a $10 million fine in restitution to identity theft victims. What happened was that over 145,000 consumers' information (including names, addresses, social security numbers, driver's license numbers, credit reports and other financial information) had been sold by ChoicePoint to scammers posing as legitimate businesses who gained access to this information.  And because of disparate laws among states in which only some state laws required notification, the Company initially disclosed the breach only to California residents.  


\ \ \ \ \ \ \ Recently, states have enacted legislation requiring notification to consumers, consumer reporting agencies (CRAs) and/or state regulators. Some of these laws allow for a private cause of action (which may require proving actual damages caused by the data breach or by a notification failure), and some have created additional laws; in particular, Minnesota enacted the "Plastic Card Security Act," which transfers the cost of card-related breaches from financial institutions to retail institutions. Additionally, because there have been more state laws proposed (and federal laws proposed which do not provide private right of action), it's important to have a general understanding.  

\ \ \ \ \ \ \ This document is just an overview of requirements, for different states, that some entities (like service providers) need to fulfill to comply with state law after a data incident or data breach.  I wanted to create a document which also gives helpful graphs and which can be updated.  This is important because while current state laws generally do not conflict among each other, they have vast differences which can make obligations to consumers unclear and ambiguous; with this in mind we are obligated to provide effective support to affected individuals within the current situation. 
