...
statesData$thresholdNotifyAG <- ifelse(is.na(as.integer(statesData$thresholdNotifyAG)) == TRUE, NA, statesData$thresholdNotifyAG)

statesData$serviceProvidersNotifyCustomers <- ifelse((str_detect(statesData$serviceProvidersNotifyCustomers, "immediately") == TRUE | str_detect(statesData$serviceProvidersNotifyCustomers, "Immediately") == TRUE), TRUE, FALSE)

statesData[is.na(statesData)] <- 0
#statesData$thresholdNotifyAG <- as.numeric(statesData$thresholdNotifyAG)
```
    ...
    inputPanel(
      ...
      selectInput("var", 
                  ...
                  choices = c(" " = toString(which(statesData$placeHolderColumn)),
                    "Date of Birth alone is PI" = toString(which(statesData$hasDOBasPI)), "Information about Online Accounts is PI" = toString(which(statesData$hasOnlineAccountsasPI)), 
...
statesDataPresentable <- statesDataSave



colnames(statesDataPresentable) <- c("State", "DOB is PI", "Online Accounts are PI", "Access", "No Private Cause of Action", "Notify AG", "Notify CRA", "Time Frame", "SP Notifies Immediately", "Harm Analysis Allowed", "Harm Analysis Required", "Financial Account Information is PI", "Notify AG Required", "Modified 2018-2019", "Is a State")
