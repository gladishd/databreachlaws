...
renderText({ 
    paste("States with the specified range(", input$range[1], " to ", input$range[2], 
           ") of Attorney General Notification Thresholds:", sep = "")
  })

renderUI({
  HTML(str_replace_all(toString(
    paste(toString(statesData[intersect(which(statesData$thresholdNotifyAG >= input$range[1]), which(statesData$thresholdNotifyAG <= input$range[2])),]$State))
    )
    , ", ", "<br/>"))
})
