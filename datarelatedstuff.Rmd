  ...
  #sidebarPanel
    inputPanel(
      ...
      selectInput("var", 
                  ...
                  "Acquisition (instead of only access) is a breach of security" = toString(which(statesData$requiresAcquisitionNotJustAccess)), 
                  "Risk-of-harm Analysis is Allowed" = toString(which(statesData$harmAnalysisAllowed)),
                  "Government Involvement is Required (that is, a private cause of action is not permitted)" = toString(which(statesData$governmentInvolvementRequired)),
...
axis_vars_graphProportions <- c(
  ...
  "Definition of Data Breach requires Acquisition (instead of just access)" = "requiresAcquisitionNotJustAccess", 
  "A Risk-of-harm Analysis is Allowed" = "harmAnalysisAllowed", 
  "Government Involvement is Required (no private cause of action)" = "governmentInvolvementRequired",
...
colnames(statesDataPresentable) <- c("State", "Modified 2018-2019", "DOB is PI", "Online Accounts are PI", "Acquisition", "Harm Analysis Allowed", "Government Involvement Required", "Notify AG", "Notify CRA", "Time Frame", "SP Notifies Immediately", "Harm Analysis Required", "Financial Account Information is PI", "Notify AG Required", "Is a State")
...
The "Financial Account Information is PI" variable doesn't refer to the financial information itself (which should always be personal information) but rather refers to information about financial accounts, information which could be used to access them.  

Financial Services account number, for instance, is considered to be personal information when it applies to students in New Hampshire.  

California, while its general breach notification statute refers to acquisition, refers to access specifically for medical information.  

With respect to what constitutes a breach of security, I elected to state that California considers access to be sufficient in some cases, namely in medical cases.  

The definition of notifying third-parties/service providers providing the information immediately was taken to be very strict - that is, phrases like "as soon as practicable", "as soon as possible" and "in the most expedient time possible" and "within 24 hours following discovery" were not taken to be the same as "immediately."  

Information about access to Online Accounts was considered alone.  
